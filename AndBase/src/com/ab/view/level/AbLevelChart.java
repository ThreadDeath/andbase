...
public class AbLevelChart extends AbLevelAbstractChart {
	...
	public void draw(Canvas canvas, int x, int y,int measureWidth,int measureHeight,int screenWidth, int screenHeight,
		...
		textLevelSize = ViewUtil.scale(screenWidth,screenHeight, textLevelSize);
		//当前值文字与顶部的距离
        ...
        int textWidth = (int)GraphicUtil.getStringWidth(textValue,mTextPaint);
		...
		for(int i=0;i<color.length;i++){
			...
			if(textlevelIndex == i){
		        ...
		        int textDescWidth = (int)GraphicUtil.getStringWidth(textDesc,mTextPaint);
			...
			if(partValue!=null && partValue.length == color.length){
		        ...
		        int partValueWidth = (int)GraphicUtil.getStringWidth(String.valueOf(partValue[i]),mTextPaint);
